...
def ds_documents(request):
    ...
    return render(request, 'main/ds_documents.html', context)


def ds_document_edit(request, document_id=None):
    """DS 문서 편집 페이지"""
    selected_category = request.GET.get('category', '')
    template_content = ''
    ...
    return render(request, 'main/ds_document_edit.html', context)
...
def ds_document_share(request, token):
...
def ds_document_share_file(request, token, file_name):
