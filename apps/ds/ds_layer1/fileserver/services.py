...
def validate_path_segment(value, label):
    """경로 세그먼트 검증 — Path Traversal 방지. 유효하면 None, 아니면 에러 메시지 반환"""
    if not value or not _SAFE_NAME.match(value):
        return f'잘못된 {label}입니다.'
    return None


def connect_fileserver_sftp():
    """파일서버 SFTP 연결"""
...
def get_country_list():
    """모니터링 대상 국가 목록 조회"""
    targets = get_monitoring_targets()
    active_countries = set(t[4] for t in targets if t[4])

    transport, sftp = connect_fileserver_sftp()
    try:
        base_path = FILE_SERVER_CONFIG['upload_path']
