...
from apps.dx.dx_layer1.views import SECTION_TITLES
...
def layer_stats(request):
    ...
    check_type_filter = request.GET.get('check_type')  # 특정 섹션만 조회 (None이면 전체)
    now = datetime.now()
    today = now.date()
    now_hour = now.hour
    ...
    try:
        ...
        if not check_type_filter or check_type_filter == 'retail':
            # ============================================================
            # 시간대 정의 - CSV에서 로드
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'sentiment':
            # ============================================================
            # TV Sentiment 검증
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'youtube':
            # ============================================================
            # Consumer (YouTube) 검증
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'market_trend':
            # ============================================================
            # Market Trend 검증
            # ============================================================
        ...
        # Market Competitor / Event 공통: 분기 계산 + 배치 조회
        if not check_type_filter or check_type_filter in ('market_competitor', 'market_competitor_event'):
        ...
        if not check_type_filter or check_type_filter == 'market_competitor':
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'market_competitor_event':
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'market_demand':
            # ============================================================
        ...
        if not check_type_filter or check_type_filter == 'market_promotion':
            # ============================================================
...
def market_competitor_raw_data(request):
    ...
    try:
        ...
        if not batch_row:
            results['columns'] = ['category', 'samsung_series_name', 'comp_brand', 'comp_series_name',
...
def market_competitor_event_raw_data(request):
    ...
    try:
        ...
        if not batch_row:
            results['columns'] = ['category', 'comp_brand', 'comp_sku_name', 'comp_launch_date',
...
# Check Log API는 Layer 4로 이전됨 → apps.dx.dx_layer4.api.views
